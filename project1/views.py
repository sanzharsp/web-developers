...
class ProductDetailView(CartMixin,View):
    ...
    def get(self, request,pk, *args, **kwargs):
        ...
        comments_get_id=Product.objects.get(id=pk).comments.all()
        comments_count=Product.objects.get(id=pk).comments.all().count()
        comments_view=Comment.objects.all()
...
def create_child_comment(request,pk):
    ...
    comments_=Product.objects.get(id=pk).comments.all()
    comment_list=create_comments_tree(comments_,pk)
    return render(request,'index.html',{'comments':comment_list})
